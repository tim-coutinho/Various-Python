...
def main():
	while True:
		try:
			...
			if score is None:
				print(f'It looks like there are no clickable links in paragraphs on that page.')
			elif score >= 0:
				print(f'It takes {score} steps to get from {raw} to Philosophy.')
				break
...
def calculate(article):
	...
	if article == '/wiki/Alphabet':
		print("You've reached Alphabet, which means you're stuck in a loop, unfortunately.")
		return -1000
	elif article == '/wiki/Branch':
		print("You've reached Branch, which means you're stuck in a loop, unfortunately.")
		return -1000
